...
def countdown_solve(numbers, result, brute_force = False, print_results = True):
    """
    A method for finding all solutions to the countdown numbers game using a meet-in-the-middle approach.
    Two solutions are only considered distinct if one cannot be transformed into the other by reordering
    the operations performed.
    Note: we allow intermediate results to be rationals and not necessarily integers.
    This works when 6 or fewer numbers are given as in the gameshow but may not show all solutions with
    more numbers.
    """
    ...
    out.extend(meet_in_the_middle(numbers, result, n, n//2-1, out, print_results))
    return out

def meet_in_the_middle(numbers, result, n, m, collisions, print_results):
    """
    Use the meet-in-the-middle approach to finding a collision between what numbers can be generated from a
    set of numbers from `numbers` of size m together with `result` and those that can be generated from the
    remaining elements of `numbers`
    """
        ...
        for key in firsts.keys():
            if key in seconds.keys():
                for method in firsts[key]:
                    if method[1]:
                        first_method = rewrite_method(first_set, method[0], result)
                        for other_method in seconds[key]:
                            second_method = rewrite_method(second_set, other_method[0], result)
                            new_method = merge_methods(first_set, first_method, second_set, second_method, result)
...
def compute_expression(numbers, expression, result, separate_sign=False):
    """
    Compute `expression` using `numbers` and `result` together with a status stating whether `result` has been
    ...
    for expr in expression:
        numbers, status, used = compute_step(numbers, expr[0], expr[1], expr[2], result, status)
        if used:
            result = numbers[-1]
    ...
    if len(numbers) != 1:
        raise ValueError("Expression did not compute only one number", numbers, expression)
    if not separate_sign:
...
def compute_step(numbers, index1, index2, operation, result, status):
    """
    Compute one step of an expression using the given indices (relative to `numbers`) and the given `operation`
    The `!` operation simply drops the second number so that it does not get used
    """

    l = len(numbers)
    if index1 < 0 or index1 >= l:
        raise ValueError('index1 not in the correct range', l, index1)
    if index2 < 0 or index2 >= l-1:
        raise ValueError('index2 not in the correct range', l-1, index2)
    new_numbers = numbers[:]
...
def rewrite_method(numbers, expression, result):
    """
    Rewrite the `expression` in terms of the actual numbers being calculated with, not their indices
    Ensure that all numbers encountered are positive
    """
    ...
    for expr in expression:
        ...
        if negative1 and negative2 and operation in '+-':
            negative = True
        step = [[numbers[index1],expr[0]], [numbers[index2],expr[1]], operation]
        numbers, status, used = compute_step(numbers, expr[0], expr[1], operation, result, status)
        if numbers[-1] < 0:
            numbers[-1] *= -1
...
def compute_all(numbers, result):
    """
    Compute all possible numbers that can be created from numbers using +, -, *, / and !
    The operator ! denotes simply dropping the second argument, allowing for not all numbers to be used
    """
    ...
    if n == 1:
        ...
        for indices in all_indices:
            ...
            D = compute_part2(counter2, parts, operations, num_exc, n, numbers, result, D)
        return D
...
def brute_force_search(numbers, result, print_results):
    ...
    if result in D.keys():
        for method in D[result]:
            ...
            new_method = [[step[0][0],step[1][0],step[2],step[3][0]] for step in new_method]
...
def merge_methods(numbers1, method1_original, numbers2, method2, result):
    """
    Merge two methods which have the same output to a single method for computing `result`
    """
    ...
    for step in method2:
        m -= 1
        method.append([[step[0][0],step[0][1]+l],[step[1][0],step[1][1]+l],step[2],[step[3][0],m]])
    used_count = 0
    using_result = [[result,l-1,used_count]]
    used_result = []
    method1_remove = []
    for step in method1:
        delta = 0
        if step[0][1] <= step[1][1]:
            delta = 1
        if step[0]+[used_count] in using_result or [step[1][0],step[1][1]+delta]+[used_count] in using_result:
            to_remove = []
            for index, triple in enumerate(using_result):
                if triple in [step[0]+[used_count],[step[1][0],step[1][1]+delta]+[used_count]]:
            ...
            for index, triple in enumerate(using_result):
                ...
                if step[1][1]+delta < triple[1]:
                    shift += 1
                using_result[index][1] -= shift
            method.append([step[0],step[1],step[2],[step[3][0],step[3][1]+1]])
    ...
    while method1:
        ...
        if step[0]+[used_count] in used_result:
            ...
            method.append([inputs[ORDER[step[2]][0]], inputs[ORDER[step[2]][1]], INVERSE[0][step[2]], step[0]])
        elif [step[1][0],step[1][1]+delta]+[used_count] in used_result:
            inputs = [step[0], step[3]]
            method.append([inputs[ORDER[step[2]][0]], inputs[ORDER[step[2]][1]], INVERSE[1][step[2]], step[1]])
        used_count -= 1
    final_method = [[step[0][0], step[1][0], step[2], step[3][0]] for step in method]
    return canonical_form(final_method, numbers1[:] + numbers2[:])

def canonical_form(method, numbers):
    """
    Write the method in a canonical form (larger values before smaller, + before -, * before /)
    ...
    while index < len(method):
        if method[index][2] == '!' and len(method) > 1:
            ...
            index += 1
    final_operation = method.pop()
    if final_operation[2] == '!':
        return str(final_operation[3])
    if final_operation[2] in '+*' and final_operation[0] < final_operation[1]:
        final_operation[0], final_operation[1] = final_operation[1], final_operation[0]
    new_method = [final_operation + [0, final_operation[2] in '+-']]
    max_depth = 0
    ...
    new_method.append([0,0,0,0,max_depth+1,0])
    final_method = []
    parts = []
    depth = 0
    ...
    while final_method:
        ...
        if part[0] == '+':
            ...
            if len(string) > 1:
                strings.setdefault(value, []).append(f'{numerator}/({denominator})')
            elif denominator:
                strings.setdefault(value, []).append(f'{numerator}/{denominator}')
            else:
